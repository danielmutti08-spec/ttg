 ...
 * - Minuscolo
 * - Senza accenti (à -> a, etc.)
 * - Trattini al posto di spazi e caratteri speciali
 */
const slugify = (text: string) => {
  return text
    .toString()
    .normalize('NFD')                   // Scompone i caratteri accentati (es. à -> a + `)
    .replace(/[\u0300-\u036f]/g, '')     // Rimuove i segni diacritici (gli accenti)
    .toLowerCase()
    .trim()
    .replace(/\s+/g, '-')                // Spazi -> trattini
    .replace(/[^\w-]+/g, '')             // Rimuove tutto ciò che non è lettera, numero o trattino
...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const initialFormState: Partial<Article> = {
    title: '', slug: '', location: '', category: Category.EUROPE,
  ...
  const handleManualSave = useCallback(() => {
    ...
    // Assicuriamoci che lo slug sia presente prima del salvataggio
    const finalSlug = formArticle.slug || slugify(formArticle.title || '');
    const submission = { ...formArticle, slug: finalSlug, id: editingId || `art-${Date.now()}` } as Article;
  ...
  const onInputChange = (key: keyof Article, value: any) => {
    let updates: Partial<Article> = { [key]: value };
    
    // Auto-generazione slug se il titolo cambia
    if (key === 'title') {
      updates.slug = slugify(value);
    }
    
    setFormArticle(prev => ({ ...prev, ...updates }));
    setSyncStatus('dirty');
  };
            ...
            <div className="col-span-8 space-y-12">
              <section className="bg-slate-50/50 p-10 rounded-[2.5rem] border border-slate-100 space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">Slug (Auto-generato)</label>
                    <input type="text" readOnly value={formArticle.slug || ''} placeholder="titolo-articolo" className="w-full px-6 py-4 rounded-2xl border-transparent bg-slate-100 outline-none text-slate-500 font-mono text-xs cursor-not-allowed" />
                  </div>
